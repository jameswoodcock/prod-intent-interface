...
start_time = 1
loop_len = 37.5      #Length of loop in seconds
...
object_level_ref = [0]*Nobjs
object_pos_ref = [0]*Nobjs

object_level_list = list()
object_pos_list = list()
object_level_list_ref = list()
object_pos_list_ref = list()

music_pos_x = [0]*len(range(27,44))
music_pos_y = [0]*len(range(27,44))
...
def send_receive_json(HOST,PORT_R,PORT_S):
    global firstLoop, metadata, metadataAdjusted, metadataInd, record, object_level_list, object_pos_list, object_level, object_pos, write_metadata, music_pos_x, music_pos_y, saved_music_pos
    # UDP socket
    try :
        ...
        if firstLoop == 1:
            ...
            for i in range(Nobjs):         #Fix heigth for forest scene
                for j in range(len(objectIDs[i])):
                        object_level_ref[i] = float(newmsg['objects'][objectIDs[i][j]]['level'])
                        object_pos_ref[i] = rad2deg(math.atan2(float(newmsg['objects'][objectIDs[i][j]]['position']['y']),float(newmsg['objects'][objectIDs[i][j]]['position']['x'])))
            ...
            if saved_music_pos != 10:
                for i in range(17):
                    music_pos_x[i] = float(newmsg['objects'][range(27,44)[i]]['position']['x'])
                    music_pos_y[i] = float(newmsg['objects'][range(27,44)[i]]['position']['y'])
                print music_pos_x
                saved_music_pos = saved_music_pos + 1
            ...
            object_level_list_ref.append(list(object_level_ref))
            object_pos_list_ref.append(list(object_pos_ref))
        else:
            if write_metadata == 0:
                with open('./' + folder_name + '/reference_metadata_level.csv',"wb") as myfile:
                    writer = csv.writer(myfile)
                    writer.writerows(object_level_list_ref)
                with open('./' + folder_name + '/reference_metadata_pos.csv',"wb") as myfile:
                    writer = csv.writer(myfile)
                    writer.writerows(object_pos_list_ref)
                write_metadata = 1                               #Run from   
                    ...
                    try:                                #Sometimes the sync goes out and inedx exceeds dimensions of object_level_list...
                        ...
                        for i in range(Nobjs):         #Cycle through object list
                            for j in range(len(objectIDs[i])):
                                ...
                                if i < Nobjs - 2:                  #Don't change position of atmos and music (they have ids < 27)
                                    if newmsg['objects'][objectIDs[i][j]]['type'] == 'plane':
                                ...
                                if i == Nobjs - 2:
                                    newmsg['objects'][objectIDs[i][j]]['position']['x'] = music_pos_x[j]
                                    newmsg['objects'][objectIDs[i][j]]['position']['y'] = music_pos_y[j]      
...
def play_loop(dt):                                              #Start playing at beginning of loop
    global firstLoop, metadataInd, thread, start_time, start_time_min
    firstLoop = 0
    metadataInd = 0
...
class mySlider(GridLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.btn_toggle_record_mode = ToggleButton(text = "Writing \nto all", state = 'normal')
        self.btn_toggle_renderer = ToggleButton(text = "Playing \nreference",state = 'normal')
        self.btn_play = Button(text = "Restart \nclip") 
    ...
    def switch_renderer(self,instance,val):
        ...
        if val == 'normal':
            ...
            renderer_thread.start()
            self.btn_toggle_renderer.text = "Playing \nreference"
        else:
            rendererFlag = 1
    ...
    def switch_record_mode(self,instance,val):
        ...
        if val == 'normal':          
            write_mode = 0
            self.btn_toggle_record_mode.text = "Writing \nto all"
    ...
    def write_data(self,dt):
        ...
        with open('./' + folder_name + '/' + str(file_name) + '_level.csv',"wb") as myfile:
                    writer = csv.writer(myfile)
                    writer.writerows(object_level_list)
        with open('./' + folder_name + '/' + str(file_name) + '_pos.csv',"wb") as myfile:
                    writer = csv.writer(myfile)
                    writer.writerows(object_pos_list)
